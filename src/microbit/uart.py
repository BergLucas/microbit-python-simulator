...
def init(
    ...
    *,
    ...
    .. warning::
        Initializing the UART on external pins will cause the Python console on USB to become unaccessible,
        as it uses the same hardware. To bring the console back you must reinitialize the UART without passing
        anything for `tx` or `rx` (or passing `None` to these arguments). This means that calling `uart.init(115200)` is
        enough to restore the Python console.
    ...
    The `parity` parameter defines how parity is checked, and it can be `None`, `microbit.uart.ODD` or `microbit.uart.EVEN`.

    The `stop` parameter tells the number of stop bits, and has to be 1 for this board.

    If `tx` and `rx` are not specified then the internal USB-UART TX/RX pins are used which connect to the USB serial converter on the micro:bit,
    thus connecting the UART to your PC. You can specify any other pins you want by passing the desired pin objects to the `tx` and `rx` parameters.

    .. note::
        When connecting the device, make sure you “cross” the wires - the TX pin on your board needs to be connected with the RX pin on the device,
        and the RX pin - with the TX pin on the device. Also make sure the ground pins of both devices are connected.
    ...
    Args:
        ...
        tx (Optional[MicroBitDigitalPin], optional): The TX pin to use. Defaults to None.
        rx (Optional[MicroBitDigitalPin], optional): The RX pin to use. Defaults to None.
    """
    ...
...
def read(nbytes: Optional[int] = None) -> Optional[bytes]:
    ...
    A bytes object contains a sequence of bytes. Because [ASCII](https://en.wikipedia.org/wiki/ASCII) characters can fit
    in single bytes this type of object is often used to represent simple text and offers methods to manipulate it as such,
    e.g. you can display the text using the `print()` function.
...
def readInto(buf: bytearray, nbytes: Optional[int] = None) -> Optional[int]:
    ...
    Returns:
        Optional[int]: The number of bytes read and stored into `buf` or `None` on timeout.
    """
    ...
...
def readline() -> Optional[bytes]:
    ...
    Returns:
        Optional[bytes]: The line read or `None` on timeout. The newline character is included in the returned bytes."""
    ...
