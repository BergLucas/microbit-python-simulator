...
class Radio:
    def __init__(
        self,
        synchronisation_address: Address,
        bluetooth_port: int,
        *,
        interval: float = 0,
        timeout: int = None,
    ...
    def on(self):
        ...
        while self.__on:
            ...
            if not Connection.isPortOpen(("127.0.0.1", self.__port), self.__timeout):
                try:
                    self.__receive_server = ConnectionServer(
    ...
    def config(self, **kwargs):
        ...
        if "length" in kwargs:
            ...
            if kwargs["length"] < 1 or 251 < kwargs["length"]:
                raise ValueError(
                    f'invalid value : {kwargs["length"]} must be between 1 and 251'
                )
        ...
        if "queue" in kwargs:
            ...
            if kwargs["queue"] < 1:
                raise ValueError(
                    f'invalid value : {kwargs["queue"]} must be greater than 1'
                )
        ...
        if "channel" in kwargs:
            ...
            if kwargs["channel"] < 0 or 83 < kwargs["channel"]:
                raise ValueError(
                    f'invalid value : {kwargs["channel"]} must be between 0 and 83'
                )
        ...
        if "power" in kwargs:
            ...
            if kwargs["power"] < 0 or 7 < kwargs["power"]:
                raise ValueError(
                    f'invalid value : {kwargs["power"]} must be between 0 and 7'
                )
        ...
        if "address" in kwargs:
            ...
            if kwargs["address"] < 0 or 2147483647 < kwargs["address"]:
                raise ValueError(
                    f'invalid value : {kwargs["address"]} is must be between 0 and 2147483647'
                )
        ...
        if "group" in kwargs:
            ...
            if kwargs["group"] < 0 or 255 < kwargs["group"]:
                raise ValueError(
                    f'invalid value : {kwargs["group"]} is must be between 0 and 255'
                )
        ...
        if "data_rate" in kwargs:
            if kwargs["data_rate"] not in (RATE_250KBIT, RATE_1MBIT, RATE_2MBIT):
                raise ValueError(
                    f'invalid value : {kwargs["data_rate"]} is must be either RATE_250KBIT, RATE_1MBIT or RATE_2MBIT'
                )
    ...
    def send_bytes(self, message: bytes):
        ...
        if len(message) > self.__length:
            raise ValueError(
                "The message has a length greater than the specified in config()"
            )
        # Send the message
    ...
    def receive_full(self) -> Tuple[Union[bytes, None], int, int]:
        ...
        return (
            self.receive_bytes(),
            self.__power,
            int((time() - self.__start_time) * 1000000),
        )
    ...
    def __acceptClientConnection(self):
        ...
        while self.__on:
            try:
                ...
                Thread(
                    target=self.__receiveData, args=(connection,), daemon=True
                ).start()
    ...
    def __refresh_connected_clients(self):
        ...
        for addr in list(self.__addr):
            ...
            if addr not in self.__connected_clients and not (
                addr[0] in self.__ips and addr[1] == self.__port
            ):
        ...
        while self.__on:
            ...
            if not self.__sync_client.connected:
                try:
                    ...
                    if self.__debug:
                        print(
                            f"Radio : Connected to the SynchronisationServer at {self.__synchronisation_address}"
                        )
                except:
                    if self.__debug:
            ...
            while self.__sync_client.connected:
                ...
                if self.__on:
                    ...
                    try:
                        ...
                        if self.__debug:
                            print(
                                "Radio : Connection lost with the SynchronisationServer"
                            )
                        break
                    ...
                    try:
                        ...
                        if self.__debug:
                            print(
                                f"Radio : Could not read the data from the SynchronisationServer {e}"
                            )
                # Disconnect if the radio is off
                    ...
                    if self.__debug:
                        print(
                            f"Radio : Disconnected from the SynchronisationServer at {self.__synchronisation_address}"
                        )
